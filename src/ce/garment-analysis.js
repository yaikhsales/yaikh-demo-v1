...
const GarmentAnalysis = ({ onBack }) => {
  ...
  const tabs = [{ id: "all", label: "Analysis List", count: 12 }];
  ...
  const handleAction = (type, name) => {
    alert(`${type} - Garment analysis details for ${name}`);
  };
          ...
          <div className="flex items-center gap-8">
            {tabs.map((tab) => (
              <button
                ...
                className={`flex items-center gap-2 pb-4 -mb-4 transition-all relative ${activeTab === tab.id ? "text-purple-600 font-black" : "text-slate-400 font-bold hover:text-slate-600"}`}
              >
                <span className="text-xs uppercase tracking-widest">
                  {tab.label}
                </span>
                <span
                  className={`px-1.5 py-0.5 rounded-md text-[9px] ${activeTab === tab.id ? "bg-purple-50 text-purple-600" : "bg-slate-50 text-slate-400"}`}
                >
                  {tab.count}
                </span>
                {activeTab === tab.id && (
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-purple-600 rounded-full"></div>
                )}
              </button>
            ))}
          </div>
