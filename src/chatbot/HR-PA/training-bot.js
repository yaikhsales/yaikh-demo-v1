...
const TrainingBot = ({ onClose }) => {
    const [messages, setMessages] = useState([
        { 
            from: 'bot', 
            text: 'Welcome to Training Module! I can help you with:\n\n1. How many training requests this month?\n2. Show me training requests approved by controller\n3. How many training requests issued to requestors?\n4. Show me training A4 items requested and issued\n5. What\'s the total training requestor requests?\n6. Give me training program statistics\n7. Show me employee training information\n\nAsk me anything about training requests!' 
        }
    ]);
    ...
    useEffect(() => {
        const saved = localStorage.getItem('training-chat-history');
        if (saved) {
    ...
    useEffect(() => {
        if (chatHistory.length > 0) {
            localStorage.setItem('training-chat-history', JSON.stringify(chatHistory));
        }
    }, [chatHistory]);
        ...
        <div className="flex flex-col h-full bg-gradient-to-br from-blue-50 to-indigo-50">
            ...
            <div className="flex items-center justify-between p-4 border-b border-blue-200 bg-white">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-semibold text-gray-800">Training Module</h2>
                </div>
                <button
                    onClick={onClose}
                    className="p-2 hover:bg-gray-100 rounded-full transition-colors"
            ...
            <form onSubmit={handleSendMessage} className="p-4 border-t border-blue-200 bg-white">
                <div className="flex items-center gap-2">
                    <input
                        ...
                        placeholder="Ask about training requests..."
