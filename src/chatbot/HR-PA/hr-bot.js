...
const HRBot = ({ onClose }) => {
    const [messages, setMessages] = useState([
        { 
            from: 'bot', 
            text: 'Welcome to HR Module! I can help you with:\n\n1. How many HR requests this month?\n2. Show me HR requests approved by controller\n3. How many HR requests issued to requestors?\n4. Show me HR A4 items requested and issued\n5. What\'s the total HR requestor requests?\n6. Give me HR department statistics\n\nAsk me anything about HR requests!' 
        }
    ]);
    ...
    useEffect(() => {
        const saved = localStorage.getItem('hr-chat-history');
        if (saved) {
    ...
    useEffect(() => {
        if (chatHistory.length > 0) {
            localStorage.setItem('hr-chat-history', JSON.stringify(chatHistory));
        }
    }, [chatHistory]);
            ...
            <form onSubmit={handleSendMessage} className="p-4 border-t border-blue-200 bg-white">
                <div className="flex items-center gap-2">
                    <input
                        ...
                        placeholder="Ask about HR requests..."
