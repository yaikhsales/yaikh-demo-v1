...
const MODULE_NAME = 'gatepass';

const GatepassBot = ({ onClose }) => {
    const [messages, setMessages] = useState([
        { 
            from: 'bot', 
            text: 'Welcome to Gatepass Module! I can help you with:\n\n1. How many gatepass requests this month?\n2. Show me gatepass requests approved by controller\n3. How many gatepass requests issued to requestors?\n4. Show me gatepass A4 items requested and issued\n5. What\'s the total gatepass requestor requests?\n6. Give me gatepass statistics\n\nAsk me anything about gatepass requests!' 
        }
    ]);
    ...
    useEffect(() => {
        const saved = localStorage.getItem('gatepass-chat-history');
        if (saved) {
    ...
    useEffect(() => {
        if (chatHistory.length > 0) {
            localStorage.setItem('gatepass-chat-history', JSON.stringify(chatHistory));
        }
    }, [chatHistory]);
    ...
    return (
        <div className="flex flex-col h-full bg-gradient-to-br from-green-50 to-emerald-50">
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-green-200 bg-white">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-semibold text-gray-800">Gatepass Module</h2>
                </div>
                <button
                    onClick={onClose}
                    className="p-2 hover:bg-gray-100 rounded-full transition-colors"
                ...
                <div className="absolute left-0 top-0 bottom-0 w-64 bg-white border-r border-gray-200 z-10 shadow-lg">
                    <div className="p-4 border-b border-gray-200">
                        ...
                        <button
                            ...
                            className="w-full px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors flex items-center gap-2"
                        >
                            <Plus size={16} />
                            New Chat
                        </button>
            ...
            <form onSubmit={handleSendMessage} className="p-4 border-t border-green-200 bg-white">
                <div className="flex items-center gap-2">
                    <input
                        ...
                        placeholder="Ask about gatepass requests..."
