...
const MODULE_NAME = 'air';

const AirBot = ({ onClose }) => {
    const [messages, setMessages] = useState([
        { 
            from: 'bot', 
            text: 'Welcome to Air Temperature & Humidity Module! I can help you with:\n\n1. How many air monitoring requests this month?\n2. Show me air requests approved by controller\n3. How many air requests issued to requestors?\n4. Show me air A4 items requested and issued\n5. What\'s the total air requestor requests?\n6. Give me air temperature and humidity statistics\n\nAsk me anything about air monitoring requests!' 
        }
    ]);
    ...
    useEffect(() => {
        const saved = localStorage.getItem('air-chat-history');
        if (saved) {
    ...
    useEffect(() => {
        if (chatHistory.length > 0) {
            localStorage.setItem('air-chat-history', JSON.stringify(chatHistory));
        }
    }, [chatHistory]);
        ...
        <div className="flex flex-col h-full bg-gradient-to-br from-purple-50 to-pink-50">
            ...
            <div className="flex items-center justify-between p-4 border-b border-purple-200 bg-white">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-semibold text-gray-800">Air Temperature & Humidity Module</h2>
                </div>
                <button
                    onClick={onClose}
                    className="p-2 hover:bg-gray-100 rounded-full transition-colors"
            ...
            <form onSubmit={handleSendMessage} className="p-4 border-t border-purple-200 bg-white">
                <div className="flex items-center gap-2">
                    <input
                        ...
                        placeholder="Ask about air monitoring requests..."
