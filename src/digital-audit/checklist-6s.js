...
import {
    ArrowLeft, Plus, Search, Eye, Edit, Trash2, X,
    CheckSquare, ClipboardList, Download, BarChart2,
    MessageCircle, AlertTriangle, CheckCircle, Clock,
    Camera, LayoutDashboard, List, FilePlus, Shield,
    RotateCcw, ChevronLeft, ChevronRight,
} from "lucide-react";
import GeneralAIAgent from "../general-ag";
import { useTranslation } from "../translate/TranslationContext";
import { DEPARTMENTS, S_CRITERIA, SCORE_LABELS, C, DEPT_CHECKLISTS } from "./checklist-6s-data";
...
const initScores = () =>
    S_CRITERIA.reduce((acc, s) => {
        acc[s.s] = { score: 0, checkItems: Array(s.items.length).fill(false), remarks: "" };
        return acc;
    }, {});
...
const calcGrade = (pct) => {
    if (pct >= 90) return { grade: "A+", cls: "text-emerald-600 bg-emerald-50 border-emerald-200", label: "Excellent" };
    if (pct >= 75) return { grade: "A", cls: "text-green-600 bg-green-50 border-green-200", label: "Good" };
    if (pct >= 60) return { grade: "B", cls: "text-yellow-600 bg-yellow-50 border-yellow-200", label: "Acceptable" };
    if (pct >= 40) return { grade: "C", cls: "text-orange-600 bg-orange-50 border-orange-200", label: "Needs Work" };
    return { grade: "D", cls: "text-red-600 bg-red-50 border-red-200", label: "Poor" };
};
...
const Checklist6S = ({ onBack }) => {
    ...
    useEffect(() => {
        try { localStorage.setItem("6s_records_v2", JSON.stringify(records)); } catch { }
    }, [records]);
    ...
    const [dashChecklistFilter, setDashChecklistFilter] = useState("All Checklists");
    const [dashCategoryFilter, setDashCategoryFilter] = useState("All Categories");
    ...
    useEffect(() => {
        try { localStorage.setItem("6s_itemAnswers_v2", JSON.stringify(itemAnswers)); } catch { }
    }, [itemAnswers]);

    const answerItem = (dept, id, field, delta) =>
        setItemAnswers((prev) => ({
            ...prev,
            [dept]: { ...prev[dept], [id]: { ...prev[dept][id], [field]: Math.max(0, (prev[dept][id][field] || 0) + delta) } },
        }));
    ...
    const clearItemImage = (dept, id) =>
        setItemImages((prev) => { const n = { ...prev }; delete n[`${dept}_${id}`]; return n; });

    /* ── scoring form ── */
    const [sTab, setSTab] = useState("S1"); // which S tab is active inside form
    const [form, setForm] = useState({ zone: "", department: DEPARTMENTS[0], auditor: "", auditDate: new Date().toISOString().split("T")[0] });
    ...
    const filtered = records.filter((r) => {
        ...
        if (q && !r.zone.toLowerCase().includes(q) && !r.auditor.toLowerCase().includes(q)) return false;
        return true;
    });
    ...
    const startEdit = (rec) => {
        setEditingId(rec.id);
        setForm({ zone: rec.zone, department: rec.department, auditor: rec.auditor, auditDate: rec.auditDate });
    ...
    const resetForm = () => {
        setEditingId(null);
        setForm({ zone: "", department: DEPARTMENTS[0], auditor: "", auditDate: new Date().toISOString().split("T")[0] });
    ...
    const handleSubmit = () => {
        ...
        if (editingId) {
            setRecords((p) => p.map((r) =>
                r.id === editingId
                    ? { ...r, ...form, totalScore: total, status, scores: JSON.parse(JSON.stringify(scores)) }
                    : r
            ));
        } else {
            setRecords((p) => [{
                id: Date.now(), ...form, status, totalScore: total, maxScore: 30,
    ...
    /* ── Score helpers ── */
    const setScore = (sKey, val) => setScores((p) => ({ ...p, [sKey]: { ...p[sKey], score: Number(val) } }));
    const toggleCheck = (sKey, idx) => setScores((p) => {
        const next = [...p[sKey].checkItems]; next[idx] = !next[idx];
        return { ...p, [sKey]: { ...p[sKey], checkItems: next } };
    });
    const setRemarks = (sKey, val) => setScores((p) => ({ ...p, [sKey]: { ...p[sKey], remarks: val } }));
    ...
    const liveAreaRows = DEPT_CHECKLISTS.map((d) => {
        ...
        const passed = d.items.reduce((s, it) => s + (dAnswers[it.id]?.yes || 0), 0);
        const failed = d.items.reduce((s, it) => s + (dAnswers[it.id]?.no || 0), 0);
        return { area: d.area, checklist: d.checklist, category: d.category, passed, failed, submissions: passed + failed };
    });
    ...
    return (
        <div className="fixed inset-0 bg-slate-50 flex flex-col z-[200] animate-in fade-in duration-400">
            ...
            <div className="bg-white border-b shadow-sm flex-shrink-0 z-[201]">
                <div className="flex items-center gap-4 px-5 py-3">
                    ...
                    <div className="flex-1 flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-lg font-bold text-slate-800 leading-tight">Checklist 6S Audit</h1>
                            <p className="text-xs text-slate-500">Sort · Set in Order · Shine · Standardize · Sustain · Safety</p>
                        </div>
                    </div>
                    ...
                    {/* <button
                        ...
                        className="flex items-center gap-2 bg-cyan-600 hover:bg-cyan-700 text-white px-4 py-2 rounded-lg font-semibold text-sm transition-colors"
                    >
                ...
                <div className="flex border-t border-slate-100 px-5">
                    {MAIN_TABS.map(({ id, label, Icon }) => (
                        <button
                            ...
                            className={`flex items-center gap-2 px-5 py-2.5 text-sm font-semibold border-b-2 transition-all ${activeTab === id
            ...
            <div className="flex-1 overflow-auto">
                    ...
                    <div className="p-6 max-w-7xl mx-auto space-y-5">
                        ...
                        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                <div>
                                    <label className="block text-xs font-semibold text-slate-500 mb-1">Filter by Area</label>
                                    <select
                                    ...
                                    >
                                        ...
                                        {liveAreaRows.map((r) => <option key={r.area} value={r.area}>{r.area}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-slate-500 mb-1">Filter by Checklist</label>
                                    <select
                                    ...
                                    >
                                        ...
                                        {[...new Set(liveAreaRows.map((r) => r.checklist))].map((c) => <option key={c}>{c}</option>)}
                                    </select>
                                </div>
                                <div>
                                    ...
                                    >
                                        ...
                                        {[...new Set(liveAreaRows.map((r) => r.category))].map((c) => <option key={c}>{c}</option>)}
                                    </select>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                                {liveAreaRows
                                    .filter((r) => {
                                        if (dashAreaFilter !== "All Areas" && r.area !== dashAreaFilter) return false;
                                        if (dashChecklistFilter !== "All Checklists" && r.checklist !== dashChecklistFilter) return false;
                                        if (dashCategoryFilter !== "All Categories" && r.category !== dashCategoryFilter) return false;
                                        return true;
                                    })
                                    .map((row) => (
                                        <div key={row.area} className="border border-slate-200 rounded-xl p-4 hover:shadow-md transition-shadow bg-white">
                                            {/* Card header */}
                                            <div className="flex items-start justify-between mb-3">
                                                <span className="text-sm font-bold text-slate-800 leading-snug pr-2">{row.area}</span>
                                            ...
                                            <div className="grid grid-cols-2 gap-3">
                                                <div className="bg-green-50 border border-green-200 rounded-lg py-3 flex flex-col items-center">
                                                    <span className="text-3xl font-bold text-green-600 leading-none">{row.passed}</span>
                                                    <span className="text-xs font-bold text-green-500 mt-1 tracking-wide">PASSED</span>
                                                </div>
                                                <div className="bg-red-50 border border-red-200 rounded-lg py-3 flex flex-col items-center">
                                                    <span className="text-3xl font-bold text-red-500 leading-none">{row.failed}</span>
                                                    <span className="text-xs font-bold text-red-400 mt-1 tracking-wide">FAILED</span>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                ...
                {/* ────── RECORDS TAB ────── */}
                {activeTab === "records" && (() => {
                    const deptData = DEPT_CHECKLISTS.find((d) => d.department === recDept) || DEPT_CHECKLISTS[0];
                    const deptAnswers = itemAnswers[deptData.department] || {};
                    const totalYes = deptData.items.reduce((s, it) => s + (deptAnswers[it.id]?.yes || 0), 0);
                    const totalNo = deptData.items.reduce((s, it) => s + (deptAnswers[it.id]?.no || 0), 0);
                    ...
                    return (
                        <div className="p-6 max-w-7xl mx-auto space-y-5">
                                ...
                                <div className="flex gap-0 overflow-x-auto border-b border-slate-100">
                                    {DEPT_CHECKLISTS.map((d) => (
                                        <button
                                            ...
                                            className={`px-5 py-2.5 text-sm font-semibold border-b-2 whitespace-nowrap transition-all ${recDept === d.department
                                                ? "border-cyan-600 text-cyan-700 bg-cyan-50"
                            ...
                            <div className="bg-white border border-l-4 border-l-cyan-500 border-slate-200 rounded-xl shadow-sm p-5 flex items-center justify-between gap-6 flex-wrap">
                                <div>
                                    <div className="text-base font-bold text-slate-800">Area: {deptData.area}</div>
                                    <div className="text-sm text-slate-500 mt-0.5">Category: {deptData.category}</div>
                                    <div className="text-sm text-slate-500">Checklist: {deptData.checklist}</div>
                                </div>
                                <div className="flex items-center gap-8">
                                    <div className="flex flex-col items-center">
                                        <span className="text-4xl font-bold text-green-500 leading-none">{totalYes}</span>
                                        <span className="text-xs font-bold text-slate-400 mt-1">Yes</span>
                                    </div>
                                    <div className="flex flex-col items-center">
                                        <span className="text-4xl font-bold text-red-500 leading-none">{totalNo}</span>
                                        <span className="text-xs font-bold text-slate-400 mt-1">No</span>
                                    </div>
                                    <div className="flex flex-col items-center">
                                        <span className="text-4xl font-bold text-blue-500 leading-none">{total}</span>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                {deptData.items.map((item) => {
                                    const ans = deptAnswers[item.id] || { yes: 0, no: 0, remark: "" };
                                    return (
                                                            ...
                                                            <label className="w-full h-32 border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center gap-1 cursor-pointer hover:border-cyan-400 hover:bg-cyan-50/50 transition-all text-slate-400 group">
                                                                <input
                                                                    ...
                                                                    onChange={(e) => {
                                                                        ...
                                                                        reader.onload = (ev) => setItemImage(deptData.department, item.id, ev.target.result);
                    ...
                    <div className="p-6 max-w-4xl mx-auto space-y-5">
                        ...
                        <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                            ...
                            <div className="p-5 grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-slate-600 mb-1.5">Zone / Area <span className="text-red-500">*</span></label>
                                    <input value={form.zone} onChange={(e) => setForm({ ...form, zone: e.target.value })}
                                        placeholder="e.g. Sewing Line A"
                                        className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500" />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-slate-600 mb-1.5">Department <span className="text-red-500">*</span></label>
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-slate-600 mb-1.5">Auditor Name <span className="text-red-500">*</span></label>
                                    <input value={form.auditor} onChange={(e) => setForm({ ...form, auditor: e.target.value })}
                                        placeholder="Auditor full name"
                                        className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500" />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-slate-600 mb-1.5">Audit Date <span className="text-red-500">*</span></label>
                            ...
                            <div className="px-5 pb-5">
                                <div className="border-2 border-dashed border-slate-200 rounded-xl p-4 flex items-center justify-center gap-3 text-slate-400 hover:border-cyan-400 hover:text-cyan-500 transition-colors cursor-pointer">
                                    <Camera size={18} />
                                    <span className="text-sm">Click to attach photos / evidence</span>
                                </div>
                            </div>
                        </div>
                                ...
                                {S_CRITERIA.map((s) => {
                                    ...
                                    return (
                                        <button
                                            key={s.s}
                                            onClick={() => setSTab(s.s)}
                                            className={`flex items-center gap-2 px-4 py-3 text-sm font-semibold border-b-2 whitespace-nowrap transition-all ${active
                                                ? `border-current ${col.text} bg-white`
                                                : "border-transparent text-slate-400 hover:text-slate-600"
                                                }`}
                                        >
                                            <span className={`w-6 h-6 rounded flex items-center justify-center text-xs font-bold text-white ${col.badge}`}>{s.s}</span>
                                            <span className="hidden md:inline">{s.label}</span>
                                            {/* mini score badge */}
                                            <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${sScore > 0 ? col.light : "bg-slate-100 text-slate-400"}`}>
                                                {sScore}/5
                                            </span>
                                        </button>
                                    );
                                ...
                                return (
                                    <div className="p-6">
                                        ...
                                        <div className="flex items-center justify-between mb-5">
                                            <div className="flex items-center gap-3">
                                                <div className={`w-12 h-12 ${col.badge} rounded-xl flex items-center justify-center text-white font-bold text-base shadow-md`}>{s.s}</div>
                                                <div>
                                                    <div className="font-bold text-slate-800 text-base">{s.fullLabel}</div>
                                                    <div className="text-xs text-slate-500">{s.description}</div>
                                                </div>
                                            </div>
                                            {/* Score selector */}
                                            <div className="flex items-center gap-3">
                                                <label className="text-xs font-bold text-slate-600">Score:</label>
                                                <select
                                                    value={sData.score}
                                        ...
                                        <div className="mb-5">
                                            <div className="flex items-center justify-between text-xs text-slate-500 mb-1.5">
                                                ...
                                                <span className="font-bold">{sData.score} / 5 ({((sData.score / 5) * 100).toFixed(0)}%)</span>
                                            </div>
                                        ...
                                        {/* Check items */}
                                        <div className="mb-5">
                                            <h4 className="text-xs font-bold text-slate-600 uppercase tracking-wide mb-3">Checklist Items</h4>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                                                {s.items.map((item, idx) => (
                                                    <label key={idx} className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all ${sData.checkItems[idx] ? `${col.ring} border` : "bg-slate-50 border-slate-200 hover:bg-slate-100"}`}>
                                                        <input
                                                            type="checkbox"
                                        ...
                                        {/* Remarks */}
                                        <div>
                                            <label className="block text-xs font-bold text-slate-600 mb-1.5">Findings / Remarks</label>
                                            <textarea
                                                rows={3}
                                        ...
                                        <div className="flex items-center justify-between mt-5">
                                            <button
                                                onClick={() => canPrev && setSTab(S_CRITERIA[sTabIdx - 1].s)}
                                                disabled={!canPrev}
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-lg font-bold text-slate-800">{calcTotal(scores)}/30</span>
                                                {(() => {
                                                    const g = calcGrade((calcTotal(scores) / 30) * 100);
                                                    return <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${g.cls}`}>{g.grade}</span>;
                                                })()}
                                            </div>
                                            ...
                                            {canNext ? (
                                                ...
                                                >
                                                    <CheckSquare size={15} /> {editingId ? "Save Changes" : "Submit Audit"}
                                                </button>
                                            )}
                                        </div>
                                    </div>
                        ...
                        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
                            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">Score Overview</h3>
                            <div className="grid grid-cols-6 gap-2">
                                {S_CRITERIA.map((s) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className={`w-8 h-8 ${col.badge} rounded-lg flex items-center justify-center text-white text-xs font-bold mb-1`}>{s.s}</div>
                                            <div className="text-xs font-bold text-slate-700">{sScore}/5</div>
                                            <div className="w-full h-1 bg-slate-100 rounded-full overflow-hidden mt-1">
                                                <div className={`h-full ${col.badge}`} style={{ width: `${(sScore / 5) * 100}%` }} />
                                            </div>
                                        </button>
                                    );
                                })}
                    ...
                    <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[92vh] flex flex-col shadow-2xl">
                        <div className="flex items-center justify-between p-5 border-b flex-shrink-0">
                            ...
                            <div className="flex items-center gap-2">
                                <button onClick={() => { startEdit(viewRec); setViewRec(null); }}
                        ...
                        <div className="flex-1 overflow-y-auto p-5 space-y-4">
                            {/* Meta */}
                            <div className="grid grid-cols-2 gap-3">
                                {[["Zone / Area", viewRec.zone], ["Department", viewRec.department], ["Auditor", viewRec.auditor], ["Audit Date", viewRec.auditDate]].map(([l, v]) => (
                            ...
                            {(() => {
                                ...
                                return (
                                    <div className={`rounded-xl p-4 border flex items-center justify-between ${g.cls}`}>
                                        <div>
                                            <div className="text-xs font-semibold opacity-70">Overall Score</div>
                                            <div className="text-3xl font-bold">{viewRec.totalScore} <span className="text-base font-normal">/ 30</span></div>
                                            <div className="text-xs mt-0.5">{pct.toFixed(0)}% — {g.label}</div>
                                        </div>
                                        <div className="text-6xl font-black opacity-20">{g.grade}</div>
                                    </div>
                                );
                            })()}
                            {/* Per-S */}
                            {S_CRITERIA.map((s) => {
                                ...
                                return (
                                    <div key={s.s} className="border border-slate-200 rounded-xl p-4">
                                        <div className="flex items-center justify-between mb-2">
                                            <div className="flex items-center gap-2">
                                                <div className={`w-8 h-8 ${col.badge} rounded-lg flex items-center justify-center text-white text-xs font-bold`}>{s.s}</div>
                                                <div>
                                                    <div className="font-bold text-slate-800 text-sm">{s.fullLabel}</div>
                                                    <div className="text-xs text-slate-400">{s.description}</div>
                                                </div>
                                            </div>
                                            <div className="text-lg font-bold text-slate-800">{sData?.score ?? 0}<span className="text-xs text-slate-400">/5</span></div>
                                        </div>
                                        <div className="h-2 bg-slate-100 rounded-full overflow-hidden mb-2">
                                            <div className={`h-full ${col.badge}`} style={{ width: `${((sData?.score ?? 0) / 5) * 100}%` }} />
                                        </div>
                                        ...
                                        {sData?.remarks && <div className="mt-2 text-xs text-slate-500 italic bg-slate-50 rounded px-2 py-1">{sData.remarks}</div>}
                                    </div>
                                );
                            })}
                        </div>
            ...
            {deleteRec && (
                <div className="fixed inset-0 z-[310] bg-black/50 flex items-center justify-center p-4 animate-in fade-in duration-200">
                    <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6 text-center">
                        ...
                        <h2 className="text-base font-bold text-slate-800 mb-2">Delete Audit</h2>
                        <p className="text-sm text-slate-500 mb-6">Delete <strong>{deleteRec.zone}</strong>? This cannot be undone.</p>
                        <div className="flex gap-3 justify-center">
                            <button onClick={() => setDeleteRec(null)}
                                className="px-5 py-2 border border-slate-300 rounded-lg text-sm font-semibold hover:bg-slate-50 transition-colors">Cancel</button>
                            <button onClick={handleDelete}
                                className="px-5 py-2 bg-red-600 text-white rounded-lg text-sm font-bold hover:bg-red-700 transition-colors">Delete</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {/* ── Bot ── */}
            <button onClick={() => setIsBotOpen(true)}
            ...
            {isBotOpen && <GeneralAIAgent onClose={() => setIsBotOpen(false)} moduleContext="Checklist 6S Audit" />}
        </div>
    );
};
