    ...
    const questions = [
        {
            ...
            content: "Does factory establish a quality policy which includes a commitment to meeting requirements and to continuous improvement of the QMS?",
            source: "Document",
            legal: "No legal reference assigned",
            explanation: "The formal quality policy was established by executive management and communicated throughout the organization. It clearly outlines the commitment to quality objectives and customer satisfaction, with established review periods for continuous improvement.",
            attachments: "3/7",
            status: "completed",
        },
        {
            ...
            content: "Is there a clear organizational chart showing the quality management structure and reporting lines?",
            source: "Chart/Document",
            legal: "No legal reference assigned",
            explanation: "The facility has a documented organizational chart that identifies personnel responsible for quality control. Reporting lines are clearly defined from production through to the Quality Manager and General Manager.",
            attachments: "2/5",
            status: "partial",
        },
        {
            ...
            content: "Are measurable quality objectives established for all relevant functions and levels within the organization?",
            source: "Document",
            legal: "No legal reference assigned",
            explanation: "SMART (Specific, Measurable, Achievable, Relevant, Time-bound) quality objectives have been set for each department. Monthly performance reviews are conducted to track progress against these targets.",
            attachments: "1/4",
            status: "completed",
        }
    ];
        ...
        <div className="flex flex-col h-screen bg-slate-100 font-sans overflow-hidden">
                    ...
                    <div className="flex items-center h-16">
                        {tabs.map((tab) => (
                            <button
                                ...
                                className={`h-full px-6 flex items-center text-sm font-semibold transition-all border-b-4 ${activeTab === tab.id
            ...
            <div className="bg-white border-b px-6 py-4 flex items-center justify-between flex-shrink-0">
                <div className="flex items-center gap-3 overflow-x-auto no-scrollbar">
                    ...
                    >
                        <RefreshCw size={16} className="group-hover:rotate-180 transition-transform duration-500" /> {t("syncToDrive")}
                    </button>
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="w-80 bg-white border-r flex flex-col flex-shrink-0 overflow-y-auto no-scrollbar">
                    <div className="p-4 bg-slate-50 border-b flex items-center justify-between">
                        <h3 className="font-bold text-slate-800 uppercase text-xs tracking-wider">{t("allAudits")}</h3>
                        <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-[10px] font-bold">782 {t("total")}</span>
                    </div>
                        ...
                        {categories.map((cat) => (
                            <div key={cat.id} className="border-b last:border-0">
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between p-4 hover:bg-slate-50 transition-colors ${selectedCategory === cat.id ? "bg-blue-50 border-l-4 border-blue-500" : "border-l-4 border-transparent"
                                        }`}
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className={`text-sm font-semibold ${selectedCategory === cat.id ? "text-blue-700" : "text-slate-700"}`}>
                                            {cat.label}
                                        </span>
                                    </div>
                                    <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${selectedCategory === cat.id ? "bg-blue-200 text-blue-800" : "bg-slate-100 text-slate-500"
                                        }`}>
                                        {cat.count}
                                    </span>
                                </button>
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-slate-100 space-y-6">
                    {questions.map((q) => (
                        <div key={q.id} className="bg-white rounded-xl shadow-sm overflow-hidden flex border hover:shadow-md transition-shadow">
                            ...
                            <div className="w-20 bg-slate-50 flex flex-col items-center pt-6 border-r flex-shrink-0">
                                ...
                                {q.status === 'completed' && <CheckCircle className="mt-4 text-green-500" size={24} />}
                            </div>
