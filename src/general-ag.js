...
import { 
    X, Send, Sparkles, Plus, Menu, Trash2, Mic, 
    Image as ImageIcon, FileText, Search, 
    Zap, BookOpen, Palette, Settings, ChevronDown,
    Paperclip, HardDrive, Camera, Notebook
} from 'lucide-react';
import { generateGeminiResponse, generateGeminiResponseWithFiles, generateImage, deepResearch } from './chatbot/gemini-api';
...
const GeneralAIAgent = ({ onClose, moduleContext = null }) => {
    ...
    const getSuggestions = () => {
        if (!moduleContext) {
            return [
                { text: 'What modules are available in Yaikh?', question: 'What modules are available in Yaikh?' },
                { text: 'How to use Purchase Request?', question: 'How do I use the Purchase Request module?' }
            ];
        }
        ...
        if (context === 'accountant') {
            return [
                { text: 'What is ACCOUNTANT module?', question: 'What is the ACCOUNTANT module and what does it do?' },
                { text: 'What are the sub-modules?', question: 'What are the sub-modules in ACCOUNTANT module?' },
                { text: 'How to verify PR?', question: 'How do I verify purchase requests in ACCOUNTANT module?' }
            ];
        }
        ...
        if (context.includes('verify pr') || context === 'verify pr') {
            return [
                { text: 'What is Verify PR?', question: 'What is Verify PR and how does it work?' },
                { text: 'How to verify requests?', question: 'How do I verify purchase requests in Verify PR?' },
                { text: 'What can I check?', question: 'What information can I check when verifying purchase requests?' }
            ];
        }
        ...
        if (context.includes('approval pr') || context === 'approval pr') {
            return [
                { text: 'What is Approval PR?', question: 'What is Approval PR and how does it work?' },
                { text: 'How to approve requests?', question: 'How do I approve purchase requests in Approval PR?' },
                { text: 'What is the approval process?', question: 'What is the approval process for purchase requests?' }
            ];
        }
        ...
        if (context.includes('pay pr') || context === 'pay pr') {
            return [
                { text: 'What is Pay PR?', question: 'What is Pay PR and how does it work?' },
                { text: 'How to pay requests?', question: 'How do I process payments for purchase requests in Pay PR?' },
                { text: 'What is the payment process?', question: 'What is the payment process for approved purchase requests?' }
            ];
        }
        ...
        if (context.includes('tb monthly') || context.includes('tb yearly')) {
            return [
                { text: 'What is TB Monthly Yearly?', question: 'What is TB Monthly Yearly and how does it work?' },
                { text: 'How to view reports?', question: 'How do I view monthly and yearly trial balance reports?' },
                { text: 'What reports are available?', question: 'What trial balance reports are available in TB Monthly Yearly?' }
            ];
        }
        ...
        if (context === 'toi') {
            return [
                { text: 'What is TOI?', question: 'What is TOI (Transfer of Information) and how does it work?' },
                { text: 'How to transfer data?', question: 'How do I transfer information using TOI?' },
                { text: 'What is global connection?', question: 'What is the global connection feature in TOI?' }
            ];
        }
        ...
        if (context.includes('factory accounting')) {
            return [
                { text: 'What is Factory Accounting?', question: 'What is Factory Accounting and how does it work?' },
                { text: 'How to manage factory accounts?', question: 'How do I manage factory-level accounting operations?' },
                { text: 'What operations are available?', question: 'What accounting operations are available in Factory Accounting?' }
            ];
        }
        ...
        if (context.includes('tax reporting')) {
            return [
                { text: 'What is TAX Reporting?', question: 'What is TAX Reporting and how does it work?' },
                { text: 'How to generate tax reports?', question: 'How do I generate tax reports in TAX Reporting?' },
                { text: 'What reports are available?', question: 'What tax reports are available in TAX Reporting?' }
            ];
        }
        
        // Default suggestions for any other module
        return [
            { text: `What is ${moduleContext}?`, question: `What is ${moduleContext} and how does it work?` },
            { text: `How to use ${moduleContext}?`, question: `How do I use the ${moduleContext} module?` },
            { text: `What features does it have?`, question: `What features are available in ${moduleContext}?` }
        ];
    };
    ...
    useEffect(() => {
        if (chatHistory.length > 0) {
            localStorage.setItem('general-ai-agent-chat-history', JSON.stringify(chatHistory));
        }
    }, [chatHistory]);
    ...
    const buildYaikhContext = useCallback(() => {
        ...
        if (moduleContext) {
            ...
            const moduleToBotMap = {
                'ACCOUNTANT': {
                    botName: 'Finance PA',
                    description: 'AI assistant for financial management, budgeting, and accounting. Handles ACCOUNTANT module including Verify PR, Approval PR, Pay PR, TB Monthly Yearly, TOI, Factory Accounting, and TAX Reporting.'
                },
                'Verify PR': {
                    botName: 'Finance PA',
                    description: 'AI assistant for Verify PR sub-module. Helps with checking and verifying purchase requests, reviewing details, invoices, and attached documents.'
                },
                'Approval PR': {
                    botName: 'Finance PA',
                    description: 'AI assistant for Approval PR sub-module. Helps with approving or rejecting purchase requests with status tracking.'
                },
                'Pay PR': {
                    botName: 'Finance PA',
                    description: 'AI assistant for Pay PR sub-module. Helps with processing payments for approved purchase requests.'
                },
                'PURCHASE REQUEST': {
                    botName: 'Admin PA',
                    description: 'AI assistant for Purchase Request module. Helps with creating and managing purchase requests, administrative operations, and related tasks.'
                },
                'YHR': {
                    botName: 'HR PA',
                    description: 'AI assistant for YHR (Human Resources) module. Helps with recruitment, interview, onboarding, attendance, benefit profile, payroll, NSSF, visa and work permit, FWCMS, and canteen management.'
                },
                'CE': {
                    botName: 'CE Assistant',
                    description: 'AI assistant for CE (Cost Engineering) module. Helps with standard time analysis, product development, garment analysis, productivity tracking, machine allocation, skill inventory, team performance, learning curves, downtimes, cost centers, CPM, and style costing.'
                },
                'YTM': {
                    botName: 'YTM PA',
                    description: 'AI assistant for YTM (Yield to Maturity) module. Helps with machine location, setup and repair, routine maintenance, system setup, data management, TV displays, transfers, downloads, analysis, and reports.'
                },
                'Salary Bill': {
                    botName: 'HR PA',
                    description: 'AI assistant for Salary Bill module. Helps with monthly salary, weekly incentive, permit fee, and resign payment management.'
                }
            };
...
IMPORTANT: 
- When responding about ${moduleContext}, ALWAYS start with **${moduleContext}:** at the top of your response
- Keep responses VERY SHORT for mockup/demo. Aim for 1-2 sentences maximum
- Be direct and to the point. Use emojis sparingly (0-1 per response)
- Format text with clear structure using bullet points when listing items
- For demo purposes: Keep responses brief and concise like a quick preview`;
        }
...
1. Any Yaikh modules and their functions - provide detailed explanations with module names
...
- When answering about a specific module, ALWAYS start with the module name in bold format: **MODULE NAME:** or **BOT NAME:**
- Examples: **ACCOUNTANT:** or **Finance PA:** or **ADMIN:** or **Admin PA:**
- Then provide the answer below the module name
- If discussing multiple modules, list each with its name at the top
- Keep responses VERY SHORT. Aim for 1-2 sentences maximum for mockup/demo purposes
- Be direct and to the point. Use emojis sparingly (0-1 per response)
- Format text with clear structure using bullet points when listing items
- For demo/mockup: Keep responses brief and concise, like a quick preview`;
    ...
    const streamBotResponse = useCallback((fullText) => {
        ...
        setMessages(prev => {
            ...
            const filtered = prev.filter(msg => !(msg.from === 'bot' && msg.isStreaming));
        ...
        const streamNextToken = () => {
                    ...
                    setTimeout(() => {
                        if (currentChatId) {
                            ...
                            const newChat = {
                                ...
                                title: updated.length > 0 && updated[0].from === 'user' ? updated[0].text.slice(0, 50) : 'New Chat',
                                messages: updated,
            ...
            // Fast for regular word tokens - adjusted for batch size
                ...
                if (tokenLength <= 3) {
                    delay = isExtremelyLong ? 3 : isVeryLong ? 4 : (5 + Math.random() * 5);
                } else if (tokenLength <= 8) {
                    delay = isExtremelyLong ? 5 : isVeryLong ? 6 : (8 + Math.random() * 7);
                } else {
                    delay = isExtremelyLong ? 8 : isVeryLong ? 10 : (12 + Math.random() * 8);
                }
            }
        ...
        setTimeout(async () => {
            try {
                setMessages(currentMessages => {
                    ...
                    responsePromise.then(geminiResponse => {
    ...
    }, [input, isTyping, streamBotResponse, attachedFiles, selectedTool, buildYaikhContext, moduleContext]);
            ...
            >
                ...
                <div className="relative w-[420px] h-[92vh] max-h-[900px] bg-black rounded-[3rem] p-3 shadow-2xl border-8 border-gray-900 flex flex-col">
                    ...
                    <div className="w-full h-full bg-white rounded-[2.5rem] overflow-hidden flex flex-col relative">
                        ...
                        <div className={`absolute inset-0 z-50 w-full sm:w-64 bg-gradient-to-br from-slate-50 to-gray-50 border-r border-gray-200 transform transition-transform duration-300 ease-in-out ${
                                            ...
                                            {chatHistory.map((chat) => (
                                                <div
                                                    ...
                                                    className={`group relative flex items-center gap-2 p-2 rounded-lg cursor-pointer hover:bg-black/5 transition ${
                                                        currentChatId === chat.id ? 'bg-gradient-to-r from-orange-100 to-amber-100' : ''
                                                    }`}
                                                >
                                                    <div className="flex-1 min-w-0">
                                                        <p className="text-xs text-gray-800 font-medium truncate">{chat.title || 'Untitled Chat'}</p>
                        ...
                        <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-gradient-to-r from-orange-500 to-amber-500 relative z-10 flex-shrink-0">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    <h2 className="text-white font-bold text-sm">{moduleContext ? moduleContext : 'Yai AI Agent'}</h2>
                                    <p className="text-white/80 text-xs">{moduleContext ? `${moduleContext} Assistant` : 'General Assistant'}</p>
                                </div>
                            </div>
                            <button
                                onClick={onClose}
                        ...
                        <div className="flex-1 overflow-y-auto p-4 bg-gray-50 relative z-10">
                            {messages.length === 0 && (
                                <div className="flex flex-col items-center justify-center h-full text-center px-4">
                                    ...
                                    <h3 className="text-lg font-semibold text-gray-800 mb-2">{moduleContext ? moduleContext : 'Yai AI Agent'}</h3>
                                    <p className="text-sm text-gray-600 mb-6">{moduleContext ? `Ask me anything about ${moduleContext} module and its sub-modules!` : 'Ask me anything about Yaikh modules, general questions, or any topic!'}</p>
                                        ...
                                        <div className="text-sm leading-relaxed whitespace-pre-wrap break-words">
                                            {msg.text.split('\n').map((line, lineIndex, array) => {
                                                ...
                                                if (/^[-•*]\s/.test(trimmedLine) || /^\d+\.\s/.test(trimmedLine)) {
                                                    ...
                                                    return (
                                                        <div key={lineIndex} className="flex items-start gap-2 mb-1.5">
                                                            <span className={`mt-0.5 flex-shrink-0 ${msg.from === 'user' ? 'text-white' : 'text-orange-500'}`}>•</span>
                                                            <span className="flex-1">
                                                                {parts.map((part, partIndex) => 
                                                                    part.startsWith('**') && part.endsWith('**') ? (
                                                                        <strong key={partIndex} className={msg.from === 'user' ? 'text-white font-semibold' : 'text-gray-900 font-semibold'}>
                                                                            {part.slice(2, -2)}
                                                                        </strong>
                                                                    ) : (
                                                ...
                                                if (trimmedLine.includes('**')) {
                                                    ...
                                                    return (
                                                        <p key={lineIndex} className={lineIndex < array.length - 1 && trimmedLine ? 'mb-2' : ''}>
                                                            {parts.map((part, partIndex) => 
                                                                part.startsWith('**') && part.endsWith('**') ? (
                                                                    <strong key={partIndex} className={msg.from === 'user' ? 'text-white font-semibold' : 'text-gray-900 font-semibold'}>
                                                                        {part.slice(2, -2)}
                        ...
                        {attachedFiles.length > 0 && (
                            <div className="px-4 pb-2 flex flex-wrap gap-2">
                                {attachedFiles.map((file, idx) => (
                                    <div key={idx} className="flex items-center gap-2 px-3 py-1.5 bg-orange-100 rounded-lg">
                                        <FileText className="w-4 h-4 text-orange-600" />
                                        <span className="text-xs text-orange-800 truncate max-w-[100px]">{file.name}</span>
                                        <button
                                    ...
                                    {isContentMenuOpen && (
                                        <div className="absolute bottom-full left-0 mb-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-30">
                                            ...
                                            >
                                                ...
                                                <span className="text-sm text-gray-800">Upload files</span>
                                            </button>
                                            <button
                                                onClick={() => {
                                            ...
                                            >
                                                ...
                                                <span className="text-sm text-gray-800">Add from Drive</span>
                                            </button>
                                            <button
                                                onClick={() => {
                                                    fileInputRef.current?.click();
                                            ...
                                            >
                                                ...
                                                <span className="text-sm text-gray-800">NotebookLM</span>
                                            </button>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="relative dropdown-container">
                                    <button
                                        ...
                                        className={`px-3 py-2 flex items-center gap-2 rounded-lg transition-colors ${
                                            selectedTool ? 'bg-orange-100 text-orange-700' : 'hover:bg-gray-100 text-gray-600'
                                        }`}
                                        aria-label="Tools"
                                    >
                                    ...
                                    {isToolsOpen && (
                                        <div className="absolute bottom-full left-0 mb-2 w-56 bg-white border border-gray-200 rounded-lg shadow-lg z-30">
                                            ...
                                            >
                                                ...
                                                <span className="text-sm text-gray-800">Create images</span>
                                            </button>
                                            <button
                                                onClick={() => {
                                            ...
                                            >
                                                ...
                                                <span className="text-sm text-gray-800">Guided Learning</span>
                                            </button>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="relative ml-auto dropdown-container">
                                    <button
                                        onClick={() => {
                                            ...
                                            setResponseSpeed(speeds[(currentIndex + 1) % speeds.length]);
                                        }}
                                        className="px-3 py-2 flex items-center gap-1 rounded-lg hover:bg-gray-100 text-gray-600 transition-colors"
                                    >
                                        <Zap className="w-4 h-4" />
                                        <span className="text-xs font-medium">{responseSpeed === 'fast' ? 'Fast' : responseSpeed === 'standard' ? 'Standard' : 'Slow'}</span>
                                ...
                                <div className="flex-1 flex items-center gap-2 bg-gray-100 rounded-full px-4 py-2">
                                    ...
                                    <button
                                        ...
                                        className={`p-1.5 rounded-full transition-colors ${
                                            isRecording ? 'bg-red-500 text-white' : 'text-gray-600 hover:bg-gray-200'
                                        }`}
